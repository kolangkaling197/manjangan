...
def get_live_stream_link(driver):
    """Mencari link m3u8 dari logs performance browser."""
    stream_url = "Not Found"
    try:
        # Bersihkan overlay iklan agar tidak mengganggu driver
        driver.execute_script("document.querySelectorAll('.modal, .popup, .sh-overlay, [class*=\"ads\"]').forEach(el => el.remove());")

        # Berpindah ke iframe player jika ada
        ...
        logging.info("    [NETWORK] Mencari link m3u8 di trafik...")
        time.sleep(12) 
...
def jalankan_scraper():
    ...
    try:
        ...
        # 1. KUMPULKAN URL DAN DATA DASAR TERLEBIH DAHULU
        ...
        logging.info(f"Ditemukan {len(targets)} card pertandingan.")

        # 2. PROSES TIAP TARGET SATU PER SATU
        for item in targets:
            try:
                ...
                logging.info(f"Mengunjungi Detail: {t1} vs {t2}")
